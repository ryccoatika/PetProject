...
final class PetView: NSView {
    ...
    func fillStroke(_ p: NSBezierPath, _ color: NSColor, _ lw: CGFloat = 1.6) {
        ...
        ink.setStroke(); p.lineWidth = lw; p.lineJoinStyle = .round; p.lineCapStyle = .round;
        p.stroke()
    }
    func oval(
        _ cx: CGFloat, _ cy: CGFloat, _ w: CGFloat, _ h: CGFloat,
        _ color: NSColor, _ lw: CGFloat = 1.6
    ) {
        fillStroke(
            NSBezierPath(ovalIn: NSRect(x: cx - w / 2, y: cy - h / 2, width: w, height: h)), color,
            lw)
    }
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        NSBezierPath(
            ovalIn: NSRect(
                x: cx - 20, y: base - 7 + min(hop, 6),
                width: 40, height: max(4, 8 - hop * 0.5))
        ).fill()
