...
extension PetView {
    ...
    func drawRunning() {
        ...
        tail(
            from: CGPoint(x: cx - 15, y: bodyY + 2),
            to: CGPoint(x: cx - 34, y: bodyY + 12 + sin(phase) * 6),
    ...
    func drawSleeping() {
        ...
        tail(
            from: CGPoint(x: cx - 20, y: base + 10), to: CGPoint(x: cx + 20, y: base + 5),
    ...
    func drawWorking() {
        ...
        tail(
            from: CGPoint(x: cx - 12, y: base + 6),
            to: CGPoint(x: cx - 30, y: base + 6 + sin(phase * 2.2) * 8),
        ...
        let lid = NSBezierPath(
            roundedRect: NSRect(x: cx - 21, y: base + 8, width: 42, height: 22),
            xRadius: 3, yRadius: 3)
        ...
        for i in 0..<3 {  // scrolling "code"
            ...
            NSBezierPath(rect: NSRect(x: cx - 17, y: base + 26 - t * 16, width: w, height: 2))
                .fill()
        }
        let deck = NSBezierPath(
            roundedRect: NSRect(x: cx - 25, y: base + 2, width: 50, height: 7),
            xRadius: 3, yRadius: 3)
    ...
    func drawAlertPose() {
        ...
        tail(
            ...
            to: CGPoint(x: cx + 6, y: base + 26 + sin(phase * 4) * 4),
    ...
    func drawCelebrate() {
        ...
        tail(
            from: CGPoint(x: cx - 13, y: base + 16), to: CGPoint(x: cx - 30, y: base + 34),
    ...
    func drawHeld() {
        ...
        tail(
            from: CGPoint(x: cx - 12, y: y - 4), to: CGPoint(x: cx - 26 + sway, y: y - 28),
        ...
        for dir in [-1.0 as CGFloat, 1.0] {
            ...
            if open < 0.2 {
                ...
                let ring = NSBezierPath(
                    ovalIn: NSRect(
                        x: ex - 4 * s, y: ey - 4.5 * s * open,
                        width: 8 * s, height: 9 * s * open))
                ink.setStroke(); ring.lineWidth = 1.3; ring.stroke()
                plainOval(
                    ex + eyeOffset.x * 1.8, ey + eyeOffset.y * 1.8 * open, 4.2 * s, 5.4 * s * open,
                    ink)
                plainOval(
                    ex + eyeOffset.x * 1.8 + 1.2, ey + eyeOffset.y * 1.8 * open + 1.6,
    ...
    func drawBangBubble() {
        ...
        str.draw(
            at: CGPoint(x: r.midX - sz.width / 2, y: r.midY - sz.height / 2), withAttributes: attrs)
    }
    ...
    func drawPill(_ text: String) {
        ...
        let sz = str.size(withAttributes: attrs)
        let r = NSRect(
            x: bounds.midX - sz.width / 2 - 7, y: 4, width: sz.width + 14, height: sz.height + 5)
    ...
    func drawZs() {
        ...
        for i in 0..<3 {
            ...
            NSString(string: "z").draw(
                at: CGPoint(x: cx + t * 10, y: base + 40 + t * 28), withAttributes: attrs)
        }
    }
}
